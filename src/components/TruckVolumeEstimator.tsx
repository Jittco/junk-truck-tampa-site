...
const ITEMS = [
  { id: "sofa", label: "3-seat sofa", cy: 1.5 },
  { id: "mattress", label: "Queen mattress + box", cy: 1.2 },
  { id: "fridge", label: "Refrigerator", cy: 1.0 },
  { id: "washer", label: "Washer/Dryer pair", cy: 1.3 },
  { id: "dresser", label: "Dresser (6-drawer)", cy: 0.9 },
  { id: "table", label: "Dining table + 4 chairs", cy: 1.4 },
  { id: "sectional", label: "Sectional couch (2 pcs)", cy: 2.2 },
  { id: "boxes", label: "10 medium boxes", cy: 1.0 },
  { id: "yard", label: "Yard debris (bagged)", cy: 0.8 },
];

function Stat({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex flex-col items-start rounded-2xl border p-4 shadow-sm">
...
function TruckLegend({ percent }: { percent: number }) {
    ...
    <div className="rounded-2xl border p-4 bg-card">
      ...
      <svg viewBox="0 0 600 120" className="mb-3 w-full">
        ...
        {segs.map((_, i) => (
          <line
            key={i}
            x1={10 + (580 / segments) * i}
            y1={20}
            x2={10 + (580 / segments) * i}
            y2={80}
        ...
        {/* Labels for eighths */}
        {segs.map((_, i) => (
          <text key={`t${i}`} x={10 + (580 / segments) * (i + 0.5)} y={100} textAnchor="middle" fontSize="10" fill="hsl(var(--muted-foreground))">
            {i + 1}/8
          </text>
        ))}
        {/* Dimensional labels */}
...
export default function TruckVolumeEstimator() {
  ...
  const itemsCY = useMemo(
    () => selected.reduce((sum, id) => sum + (ITEMS.find((i) => i.id === id)?.cy || 0), 0),
    [selected]
  );
  ...
  const truckFractionSteps = [
    { label: "Min. / Item", fraction: 0.08 },
    { label: "1/8 Truck", fraction: 0.125 },
    { label: "1/4 Truck", fraction: 0.25 },
    { label: "3/8 Truck", fraction: 0.375 },
    { label: "1/2 Truck", fraction: 0.5 },
    { label: "5/8 Truck", fraction: 0.625 },
    { label: "3/4 Truck", fraction: 0.75 },
    { label: "7/8 Truck", fraction: 0.875 },
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-4">
      <h2 className="text-2xl font-anton text-foreground">Visual: How Much Space Will Your Items Fill?</h2>
